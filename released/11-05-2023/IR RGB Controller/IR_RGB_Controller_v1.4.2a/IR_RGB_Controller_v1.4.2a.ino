/*
  Library Requirements!
    - Arduino IRremote libaray
      by [Armin Joachimsmeyer]
      ver [4.1.2](tested)

  ToDo:
    - Fix white color cycling
    - Implement lightweight Serial Debugging
        Not enough storage
        To reenable Serial debugging, select all instances of 'Serial' and uncomment
    - Cleanup and document
    - Move led handle to interrupt if possible
        Some unwanted jitter may occur with so much processing
        *may interfere with IR input
*/

#include "Arduino.h" // default

// IR codes corresponding to each Remote button:
#include "other/IR_codes.h"

// pallette with default colors and sequences:
#include "other/ColorPallette.h"

// include IR Remote for parsing IR Signals (This library must be installed):
#include "other/IR_config.h"  // << this is use together with the IRremote library
#include <IRremote.h>

// include output macros and sequence build for LED rendering:
#include "src/SequenceBuild/SequenceBuild_1.0.4.h"
#include "src/LedMacro/LedMacro_1.1.0b.h"

// include input macro and VarPar for help with IR Inputs:
#include "src/InputMacro/InputMacro_1.0.1.h"

// include EEPROM linked list for storing new macro sequences:
#include "src/FixedList_EEPROM/FixedList_EEPROM_1.0.0.h"  // ToDo, finish linked list EEPROM
...
void setup()
{
  ...
  // init IR receiver:
  IrReceiver.begin(IR_RECEIVE_PIN, false /*(IR debug pin disabled)*/);
...
void loop()
{
  ...
  Led_handle();
  IR_handle();
  // Logic_handle();  // running this is handled by IR_handle
  SpeedAdjust_handle();
}
...
void IR_handle()
{
  ...
  if (!logicRan)
  {
    if (millis() - __logicTimer__ >= 33) // << ~30hz default
    {
      Logic_handle();
      logicRan = true;
      __logicTimer__ = millis();
...
void volatile Logic_handle()
{
  ...
  */
  ...
  ////// Master On/Off Commands //////
  {
    if (ir_commandMacroState_on)
    {
      ...
      case IrCode_on:
        ...
        Macro.quadEase(MasterOnOff_val, 255, 800); // was 1000ms
      ...
      case IrCode_off:
        ...
        Macro.quadEase(MasterOnOff_val, 0, 1000); // was 500ms
        masterOff_Sequence();
        break;
      }
    }
          ...
          if (brightnessDimmer_enable)
          {
            if (MasterOnOff_state)
            {
              ...
              else
              {
                ...
                // only run flash if not specific macro runs:
                if (WorkMode != WorkMode_MacroRun)
                  LedBuild[3].setPrioritySequence(DimmerFlash, 0, true);
                else
                {
                  switch (MacroMode)
                  {
                  case MacroMode_Smooth: // only smooth really looks okay with this
                    LedBuild[3].setPrioritySequence(DimmerFlash, 0, true);
                    break;
                  }
                }
      ...
      if (checkIrColor(IR_PressedButton) != -1) // check if valid color command
      {
        ...
        else
        {
          ...
          case WorkMode_Static:
            ...
            runIrColor(IR_PressedButton, 800, 0);
            break;
      ...
      case IrCode_smooth:
        ...
        case WorkMode_MacroRun:
          if (ir_commandMacro && ir_commandMacro.interval() > macroStoreStateHoldTime && !ir_commandMacro.triggered())
          {
            ir_commandMacro.trigger();
            if (WorkMode == WorkMode_MacroRun)
              MacroSequence_Stop(); // macros currently running, must be stopped
  ...
  {
    const uint8_t speedAdjust_increment = 10; // speed adjust increment amount, must be greater than 1
    ...
    if (MacroSpeedAdjustState)
    {
      if (!ir_commandMacro)
      {
        if (ir_commandMacro.interval() > macroSpeedTimeOut) // no need to trigger, 'MacroSpeedAdjustState' will take care of that
        {
          ...
          {
            MacroSpeedAdjustState = false; // Exit Speed Adjust
            // LedBuild[3].setPrioritySequence(SpeedAdjustExitFlash, 0, true);  // maybe? If so, make sure only on specific sequences
...
void SpeedAdjust_handle()
{
  ...
  case WorkMode_MacroRun:
    ...
    if (millis() - timer >= 90) // << about 11hz
    {
      ...
      {
      case MacroMode_Flash:
        newSpeed = map_f(SpeedAdjust_val, 0.2, 8);
        break;
      case MacroMode_Strobe:
        newSpeed = map_f(SpeedAdjust_val, 0.1, 9);
        break;
      case MacroMode_Fade:
        newSpeed = map_f(SpeedAdjust_val, 0.1, 15);
        break;
      case MacroMode_Smooth:
        newSpeed = map_f(SpeedAdjust_val, 0.1, 15);
        break;
