...
void Logic_handle()
{
    ...
    default:
      ...
      case WorkMode_MacroStore: // run colors that will be used to store to new macro
        if (checkIrColor(IR_PressedButton) != -1)
        {
          ...
          LedBuild[1].reset();
          LedBuild[1].setSequence(MacroStoreFlash, 0);
    ...
    if (!ColorEditState && WhiteCycleEnable)
    {
      ...
      case IrCode_cont_white:
        ...
        case WorkMode_MacroStore:
        {
          ...
          // set sequence to pause flashing
          LedBuild[1].reset();
          LedBuild[1].setSequence(MacroStoreFlash, 0);
    ...
    if (!ColorEditState)
    {
      ...
      case WorkMode_MacroStore:
        ...
        case IrCode_smooth:
          ...
          LedBuild[1].reset();
          LedBuild[1].setSequence(MacroStoreFlash, 6);
          ir_commandMacro.trigger();
          break;
        }
        break;
  ...
  //// Macro Store mode buttons ////
  const uint16_t macroStoreHoldTime = 1000;
  switch (IR_PressedButton)
  {
  case IrCode_flash:
  case IrCode_strobe:
  ...
  case IrCode_smooth:
    ...
    {
    case WorkMode_Static:
      if (!ColorEditState && MasterOnOff_state == 1) // make sure master power is on and not in color edit state, prohibited state!
      {
        if (ir_commandMacro && ir_commandMacro.interval() > macroStoreHoldTime && !ir_commandMacro.triggered())
        {
          ir_commandMacro.trigger();
          WorkMode = WorkMode_MacroStore;
          LedBuild[1].reset();
          LedBuild[1].setSequence(MacroStoreFlash, 1);
...
void StatusLed_handle()
{
  ...
  {
    /*
      set normal sequence, Color edit sequence will be run as priority.
      As soon as Color edit is done, this sequence will take over and restart
    */
    LedBuild[1].setSequence(MacroStoreFlash, 1);
  }
}
...
SB_FUNCT(MacroStoreFlash, Macro.ready(Brightness_val[1]) &&
                              Macro.ready(RedLed_val[1]) &&
                              Macro.ready(GreenLed_val[1]) &&
                              Macro.ready(BlueLed_val[1]) &&
                              Macro.ready(WhiteLed_val[1]))
...
SB_STEP(
    ...
    const uint16_t setDelay = 200;
    Macro.stop(Brightness_val[1]);
    Macro.set(RedLed_val[1], 255, setDelay);
    Macro.set(GreenLed_val[1], 255, setDelay);
    Macro.set(BlueLed_val[1], 255, setDelay);
    Macro.set(WhiteLed_val[1], 255, setDelay);)
// index 2:
SB_STEP(
    const uint16_t setDelay = 200;
    Macro.stop(Brightness_val[1]);
...
SB_STEP(
    ...
    Macro.delay(WhiteLed_val[1], setDelay);)
// index 4:
SB_STEP(if (WorkMode == WorkMode_MacroStore) _this->loop(1);)
// index 5:
SB_STEP(
    Brightness_valSelect = 0;
    RedLed_valSelect = 0;
    ...
    WhiteLed_valSelect = 0;
    _this->reset();)
// begin from here to end macro store mode with a double flash:
// index 6:
SB_STEP(
    Brightness_valSelect = 0;
    RedLed_valSelect = 1;
